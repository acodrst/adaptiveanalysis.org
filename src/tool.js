    ...
    if (line.slice(0, 2) != "//") {
      ...
      if (line.includes("::")) {
        ...
        [
          ...
          ? structured.push({ x: 1, y: row, w: 3, h: 1, content: line })
          : ["forward", "back", "both"].includes(last_command)
            ? structured.push({ x: 2, y: row, w: 3, h: 1, content: line })
            : ["items"].includes(last_command)
              ? structured.push({ x: 2, y: row, w: 3, h: 1, content: line })
          ...
          if (
            ...
            if (["transforms", "processes"].includes(last_command)) {
              ...
              dots[level.join(".")][line] = zoom == "zoomnotable"
                ? `"${line}" [id="${rn()}" color="#33bbee" shape="rectangle" style="rounded" class="${last_command} ${zoom}" label="${res.path}\n${l_lbl}"]`
                : `"${line}" [id="${rn()}" color="#33bbee" href="#${res.dpath}" shape="rectangle" style="rounded" class="${last_command} ${zoom}" label="${res.path}\n${l_lbl}"]`;
            }
...
let dots = model_to_dots(Deno.readTextFileSync("assets/model.txt")).dots;
for (let i in dots) {
  const dot = `digraph {
...
const p_c = ['--highlight-style=kate', '--pdf-engine=lualatex','--embed-resources=true', '--filter',
  'pandoc-crossref', '--citeproc', '-o', 'assets/description.pdf', 'assets/description.md','metadata.yaml']
